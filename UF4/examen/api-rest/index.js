...
app.post('/events/add', function (req, res) {
  ...
  app.post('/subscription', (req, res) => {
    ...
    const insertQuery = 'INSERT INTO users (username, userpass, role) VALUES (?, ?, "cliente")';
    connection.query(insertQuery, [email, password, role], (error, results) => {
        if (error) {
            console.error('Error inserting data:', error);
            res.status(500).send({ status: 'error', message: 'Error registering user.', error: error.message });
        } else {
